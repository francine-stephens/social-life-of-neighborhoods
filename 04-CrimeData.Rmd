...
## Visualizing aggregated incidents
...
One quick and easy way to convey the concentration and distribution of the point pattern is to change the symbology to a **"heat map".** A heat map is a very generalized form of a point density map. *This map is good for drawing attention to where points are concentrated the most.* Where the color is "hot" (on the legend), the incidents are concentrated the most, and where the color is "cold" represents fewer or no incidents. 
...
![Create a Heat mapping](images\crime_heat_map.gif)
...
### Hot Spot & Graduated Symbol Maps

To visualize the concentration of crime incidents by location in a more detailed, quantitative fashion, we would want to use a **graduated symbol map,** where the size of the symbol indicates the number or range of numbers of incidents that occured in the location. The first step in creating the graduated symbol map is to "aggregate" the incidents. Unlike other GIS software, AGO does not have tool to aggregate points based on point location. The best we can do with AGO is to aggregate to a polygon. For this reason, we will use a **Hot Spot Analysis** to aggregate to a series of small polygons that cover the area of our neighborhood. Aggregating to small polygons makes sense given that the points were blurred by the police department and represent the approximate locations of crimes. Thus, the polygons can represent the potential area where the incidents may have occurred.

The hot spot analysis will output a map of hot spots, but we will end up changing the attribute to the point counts and visualize the counts with a different drawing symbology to get a graduated symbol map.
...
The hot spot map, shown below, is not really what we are trying to map. We are interested in the frequency of crime incidents in these locations, so we need to take one more step to get there. However, it feels a little weird to not provide any background on a hot spot map. That is why I have included a brief description of hot spots and what they mean below. I am not including any information about how to clean up the styling of the hot spot map because my guess is that the graduated symbol map is going to be far more useful to most of the class than the hot spot map. If you find that the hot spots are helpful for conveying the points you are trying to make in your assignment write-up, and want help with the styling contact [Francine](mailto:fis@stanford.edu) or come to office hours.

![Crime hot-spots map layer](images\crime_hot_spots_legend.png)


**What makes a hot spot map different from a heat map?** The short answer is statistics! A heat map maps how concentrated points are across the space in your viewing frame, but a hot spot map compares how concentrated points are in sub-regions of the area of interest (e.g., neighborhood) to how concentrated points would be if the points were distributed in a completely random manner across the area of interest. From the map, you can see which aggregated regions (hexagons or squares, depending on your grid) have more or less incidents than we would expect if the "crime generating process" was completely random. So, the hot spots indicate regions where the frequency of crimes is higher than we would expect if crime was randomly distributed across the neighborhood. Cold spots would represent regions with frequencies that are lower than expected if crime was randomly distributed across the neighborhood. The darker the shading indicates higher levels of statistical confidence in the hot spot estimates.


Now, let's press onward to making the graduated symbol map for crime! As you might guess, we want to select the **Change style button** on the hot spot layer. Follow the steps below: 

1. In the drop-down box, change the **Attribute to Number of Points.** You should see your map's visualization style change to circles of different sizes. The legend also shows how the sizing relates to the number of points.
2. Click on the **Options** for **Counts and Amounts (Size) style**.
3. **[Optional] Change the symbol.** I like the fireflies symbol because the glow kind of mimics the aesthetic of the heat map in point form. However, other symbols work fine, too.
4. Once you are happy with the symbol, go to the **Polygons symbol** and set the **Outline to No Color.**
5. Lastly, check the **Classify Data radio box** and set the **Using option** as **Equal interval.** This makes the frequency bins for each symbol level of equal size. 
6. Also, **Play with the number of classes.** Four may leave you with really large frequency bins, so you will want to increase. Be mindful that creating a large number of classes may make it trickier for the reader to discern differences in the size of the symbol. The rule of thumb is to generally create more than 7 classes. 
7. Scroll back up to change the range of the legend by clicking on the zero below the number of points legend scale and set it to 1. This will ensure that the map does not put a point in locations where there are no crime incidents recorded.
8. **Round** your classes to the nearest 1 or 10 (depending on your data).
9. **[Optional] Change transparency** if you are choosing a colored shape symbol.
10. When you are happy with the drawing style of the symbology, **select Okay, and then Done.**

You will end up with a map that looks something like the screen shot below.

![Graduated symbol map](images\crime_graduated_symbols.png)

Now, we have a map that tells us something about the number of crime incidents in different areas of our neighborhood over the timespan covered by the crime data For San Francisco, this represents the number of crimes since January of 2020. We can see that the area near Geneva Ave and Mission St. tends to have the highest number of crime incidents over this time period. 

One thing to note about this visualization approach is that the points represent the centroid of the hexagon areas. It is common practice to represent polygon areas that aggregate points by the polygon's center point.  However, note that these centroids distribute the points across the study area more than the street intersections or street midpoints. The other thing you might notice is that a few points may show up outside of your neighborhood. That is because the hexagon or square cells in your hot spot grid may have had their center point outside of your neighborhood boundaries. You do not need to clean this up. It is not unreasonable to imagine that even some of the blurred crime locations that you imported actually occurred on the edge or just outside of your neighborhood boundaries. 
...
Notice that your map has changed. The high heat areas may reduce in intensity or disappear all together. You should inspect your map for patterns, and try visualizing using the hot spot tool and graduated symbols map like we did with the full dataset in the previous sections. Pay attention to how this particular set of crimes compared to the full set of crimes and how it compares to the elements of public order/disorder that you observed during your walk through the neighborhood or on Google Street View. 
...
As mentioned in the previous section, you would want to consider the an appropriate visualization technique for your filtered points. There are a couple of things to be mindful of when it comes to visualizing the filtered data. 

**First, if your filtering yields fewer than 30 crimes, do not use the hot spot tool.** Check to see if you have any points that are co-located (i.e., on top of one another). If you do not have co-located points, just map the locations (not counts/amounts). If you do have co-located points, just use the heat map and adjust the drawing style for the High-Low scale and the area of influence to more clearly show which locations multiple incidents of this type. 

**If you have at least 30 crimes, but fewer than 60 crimes in your neighborhood after filtering, there is a small chance that the Find Hot Spots tool will not work using the settings described in the section above** on [hot spots and graduated symbol maps][Hot Spot & Graduated Symbol Maps]. The tool can be picky about the size of the area of interest (our neighborhood) when there are fewer than 60 points. **Try using the drawing tool** next to the field labeled, **Define where points are possible** to draw a polygon that encloses the neighborhood boundaries. If you get an error message, email or meet with [Francine](mailto:fis@stanford.edu) to do a little more trouble-shooting.
...
This tutorial walked you through creating multiple layers. For both aesthetic and readability reasons, it is not necessarily ideal to show all of the layers that you created on the same map at once. Assignment #4 does not require that you use all of these map layers in your write-up. Instead, you might consider using a couple of maps to reveal the patterns (or lack thereof) that you find compelling or interesting and want to discuss in your write-up. I could see one map for showing the general/aggregated distribution of crime incidents with layers for your social order/disorder elements and key organizations. The second map would feature the disaggregated crime characteristics that you chose to focus on (e.g., type of crime) alongside layers for your social order/disorder elements and key institutions. This is where the comment above about **Saving As** after making the filtered map comes in handy.

For the map with the generalized/aggregated crime data, you do not need to have the heat map layer and the graduated symbol layer both visible. Even if you have both layers turned on, it may be hard to view them together. Determine which visualization style better supports the patterns you are discussing in your write-up. The heat map is good for conveying generalized/high-level patterns, so if you are focused on broad regional differences within your neighborhood (e.g., the difference in crime prevalence between areas east and west of Main Street), then the heat map may be a good choice. On the other hand, if you want to quantify differences in crime incidents at a finer-grained spatial level, then the graduated symbol map would be the better option. 

The hot spot map does not need to be included in your write-up. The hot spot tool/map was a map layer we had to use to get to the aggregated points. If you want to make a statement about statistically significant patterns of crime in your neighborhood, you can use the map layer/reference it in your write-up. Just know that even if an area does not have a statistically significant hot or cold spot, it does not mean there are not meaningful or substantively interesting patterns occurring there. **There are plenty of patterns in the real world that are not statistically significant, but still compelling/interesting/surprising.**
...
Hopefully you enjoyed getting to try out different visualization approaches this week. Next week we will have another opportunity to play with visualization strategies when we map segregation trends and changes in racial composition within the neighborhood.
