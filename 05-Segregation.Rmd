...
Schafran also takes issue with using the Census Bureau's Metropolitan Statistical Area (MSA) as a region. The geographic extent of MSAs varies across the U.S. because they are drawn according to population size thresholds. Schafran thinks that commuting patterns are an important dimension of a region, which is what he uses to define 12 counties that make-up the Bay Area region. It is tricky to apply his approach to defining a region to other parts of the country, so regions are going to be defined differently based on where your neighborhood is located. If your neighborhood is in the Bay Area, your region extent will be the same as Schafran's. If your neighborhood is located somewhere else, we will just use the combined-statistical area geography from the U.S. Census Bureau.  Check-out the table below to see how it has been defined. 
...
| Chino       | Riverside-San Bernandino-Ontario, CA |
...
![You can make a choropleth map for proportions of a particular racial group like Schar](images\recreate_Scharfran_map_pblack.png)

As you map the places in the region, especially if your region is large like the Bay Area, you might find that the time animator may struggle to render the map (it might lag for some places or not show them). You can see this in the GIF below. You have two options to work around this problem. One is to just pause at each decade and make note of your observations (we can do that too when you submit your maps). The other option is to use a **filter** like you did with the crime data and create separate layers for each decade in our time span. You can turn on and off the layers manually and make note of your patterns.  
...
Open up a new Map Viewer window and set-up your basemap with the style you prefer. Then, navigate to the **Add Layers** button and search for the places layers in your region with segregation data [Naming style: "Region_Name_places_segregation"]. Add the layer to the map. The layer contains the dissimilarity index scores and divergence index score for all places in the region at each decade. When you interpret the index scores, remember that your **sub-unit is the place** and the **super-unit is the region**.

You can visualize both types of indices using the same choropleth and categorical mapping techniques demonstrated in the section above where we [mapped the Divergence index][Divergence Index Choropleth Map]. If the time animator was acting up for your regional composition map, you will have the same problem again with the segregation map. Use one of the two work-arounds mentioned above to get by for this assignments. We will come back and tweak these maps at the end of the quarter to fold into the Story Maps.  

When mapping segregation, **be mindful of the fact that you cannot represent the dissimilarity index for all race pairs (e.g., Black-white, Asian-white, Hispanic-white, etc.) at the same time on a single map.** If your focus is on evenness of the distribution of race groups in places across the region, then you would need to pick the group pairs that you want to speak to in your write-up and map them as separate choropleth/categorical layers.  
