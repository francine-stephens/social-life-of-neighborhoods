...
The dissimilarity index is minimized (i.e., equals 0), when the proportion of each group in the census tract matches the proportions of the group in the city. By contrast, a score of 100 would signal that the tract is home to only one racial/ethnic group.
...
```{r dissimilarity interpretations}
dissim_scores <- tribble(
  ~Level,  ~Score,  
  "Low", "(0, 30]", 
  "Moderate", "(30, 60]",
  "High",  "(60, 100]"
...
knitr::kable(dissim_scores,
             #col.names = gsub("[_]", " ", names(dissim_scores)),
             caption = "Dissimilarity Index: Levels of Segregation")
...
```{r divergence interpretations}
diverg_scores <- tribble(
  ~Level,  ~Score,  
  "Low", "[0, 0.1]", 
  "Moderate", "(0.1, 0.25]",
  "High",  "(0.25, 1.0]"
...
knitr::kable(diverg_scores,
             #col.names = gsub("[_]", " ", names(dissim_scores)),
             caption = "Divergence Index: Levels of Segregation")
...
With an index measure like the divergence index, the changes in the score over time are not clearly communicated using a choropleth map with the full numeric scale. It can be easier to spot the neighborhoods (i.e., census tracts) experiencing changes in segregation by reclassifying them into categories of low, moderate, and high, like the ones listed in the table above for segregation levels. Let's do this with our map! Return to the **Change Styles** page for the layer and again select the **Options** button again on the color drawing style. 
...
2. Select the **Manual Breaks** Classification as the **Using** option from the drop down menu, and enter **3** for the **number of breaks.** 
3. We want to match the 3 levels to match those in the table above, so go to the legend and change the values for the breaks to match the ones in the table. 
...
**NOTE: This section is being updated due to some AGO data rendering issues. The section will be live by 4/27/2020**
