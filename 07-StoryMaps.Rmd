...
* [Francine's](https://arcg.is/1Xj0v4) example story map for San Francisco's Excelsior neighborhood. This is a work-in-progress (i.e., There are some slides with place-holders for visuals). 



## Using Time-Series Maps in StoryMaps

Time can be displayed You may want to use one of the time series maps that you created for the racial composition or segregation in your story map. When you created those maps, we used the time animator to sequentially show each decade as a layer on the map. Unfortunately, you cannot just load those web-maps with the time animator directly into the story map template. Instead, you need to make a choice about whether you want to display time in an animated fashion or if you would rather have your reader scroll or click through each time period in your story map. 

If you want the **time animator** in your story map, you will need to use either a journal or map series template. Go into your web-map with the time animator and **create a time-aware app** using the map. You can find instructions about how to create the time aware app [here](https://www.esri.com/arcgis-blog/products/arcgis-online/uncategorized/how-to-make-a-time-aware-story-map/?rmedium=redirect&rsource=blogs.esri.com/esri/arcgis/2015/08/03/how-to-make-a-time-aware-story-map). If you are having trouble with this, email/meet with Francine. 

*One caveat to this is that the time-aware app is that it does not work on maps created with the New Map Viewer (e.g., dot density maps like the racial composition maps you made).* For these maps, you will need to use the second option of having separate layers for each decade that viewers scroll or click through.

The other option for displaying time-series data requires you to disable time on your map and **use the filter option to create separate layers** for each decade that you want to display to your audience. You can load these separate layers into any story map template and have your viewer scroll or click through them. *If you are working with the racial composition dot-density map (created in the new map viewer), share these layers as separate web-apps from the New Map Viewer page.* You can embed the links on your apps as webpages in your story map.  
...
* [Working with time in storymaps](https://www.esri.com/arcgis-blog/products/arcgis-online/uncategorized/how-to-make-a-time-aware-story-map/?rmedium=redirect&rsource=blogs.esri.com/esri/arcgis/2015/08/03/how-to-make-a-time-aware-story-map)
